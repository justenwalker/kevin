...
func getOrigDst4(fd int) (netip.AddrPort, error) {
	...
	if err := getsockopt(fd, unix.SOL_IP, unix.SO_ORIGINAL_DST, unsafe.Pointer(&raw), uint32(unsafe.Sizeof(raw))); err != nil { //nolint:gosec // raw is a fixed-size unix.RawSockaddrInet4, sized by unsafe.Sizeof itself; the kernel writes exactly that many bytes
...
func getOrigDst6(fd int) (netip.AddrPort, error) {
	...
	if err := getsockopt(fd, unix.IPPROTO_IPV6, ip6tSoOriginalDst, unsafe.Pointer(&raw), uint32(unsafe.Sizeof(raw))); err != nil { //nolint:gosec // raw is a fixed-size unix.RawSockaddrInet6, sized by unsafe.Sizeof itself; the kernel writes exactly that many bytes
...
func getsockopt(fd, level, opt int, buf unsafe.Pointer, bufLen uint32) error {
	_, _, errno := unix.Syscall6(unix.SYS_GETSOCKOPT, uintptr(fd), uintptr(level), uintptr(opt),
		uintptr(buf), uintptr(unsafe.Pointer(&bufLen)), 0) //nolint:gosec // bufLen is a local stack variable read back by the same syscall that writes into buf; its address never escapes
	if errno != 0 {
		return errno
	}
	return nil
