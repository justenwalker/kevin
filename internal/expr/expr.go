...
func buildEnv() (*cel.Env, error) {
	env, err := cel.NewEnv(
		cel.Variable("needs", cel.MapType(cel.StringType, cel.MapType(cel.StringType, cel.MapType(cel.StringType, cel.StringType)))),
...
// The value's strings are evaluated against two variables:
...
//  2. `env`: the kevin process's own environment variables, e.g. `env.HOME`.
//     A reference to an unset variable errors; use `has(env.FOO) ? env.FOO : "default"` for a fallback.
func Render(raw json.RawMessage, step string, deps, system map[string]dag.Outputs) (json.RawMessage, error) {
	...
	env := hostEnv()
	rendered, err := renderValue(v, step, needs, env)
	if err != nil {
		return nil, err
	}
...
func renderValue(v any, step string, needs map[string]any, env map[string]string) (any, error) {
	switch t := v.(type) {
	case string:
		return renderString(t, step, needs, env)
	case map[string]any:
		for k, elem := range t {
			rendered, err := renderValue(elem, step, needs, env)
			if err != nil {
				return nil, err
			}
			t[k] = rendered
...
func renderString(s string, step string, needs map[string]any, env map[string]string) (string, error) {
	...
	for {
		...
		result, err := eval(exprStr, step, needs, env)
		if err != nil {
			return "", err
		}
...
// eval compiles and evaluates one CEL expression against needs and env, and
// requires the result is a string.
func eval(exprStr, step string, needs map[string]any, env map[string]string) (string, error) {
	cEnv, err := celEnv()
	if err != nil {
	...
	out, _, err := prg.Eval(map[string]any{"needs": needs, "env": env})
	if err != nil {
