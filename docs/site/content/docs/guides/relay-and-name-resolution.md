...
This registration is now redundant for a kind pod's own egress, the same way it already was for a `builtin:container` step's: a pod's traffic is captured at the node it runs on regardless of which address it actually resolved (see [Transparent capture]({{< relref "/docs/concepts/relay#transparent-capture" >}})), so `s3.us-east-1.amazonaws.com` would land on the interception whether or not the relay ever spoofed an answer for it - a pod dialing the real S3 IP on port 443 is still traffic transiting its node on a captured port. The registration stays anyway (kept, not deleted, since it's harmless and correct, just no longer load-bearing); what genuinely can't be replaced by capture is resolving the *environment domain* itself, `<step>.kevin.home`, which has no real DNS record anywhere - only CoreDNS's forward zone to the relay makes that resolve at all.
