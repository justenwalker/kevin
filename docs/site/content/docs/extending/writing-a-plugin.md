...
- **`plugin.Exporter`**: a step type whose `Up` creates something reachable from outside kevin implements `Export(ctx, *plugin.ExportRequest) (*plugin.ExportResult, error)`. `ExportResult` carries `Out map[string]plugin.Value`, the same shape `Result.Outputs` uses (`plugin.String`, `plugin.Sensitive`, `plugin.StringMap` all apply) - read by another step's cross-scope `needs: ["setup.<name>"]` reference (see [Environment file: reading a setup step's value]({{< relref "/docs/environment-file#reading-a-setup-steps-value" >}})), and by a `commands:` entry's `run` through `kevin do` (`"${needs.<step>.out.<key>}"`). `Export` must not create or change anything, only report how to reach what `Up` already created.
