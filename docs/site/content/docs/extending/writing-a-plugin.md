...
- **`plugin.Exporter`**: a step type whose `Up` creates something reachable from outside kevin implements `Export(ctx, *plugin.ExportRequest) (*plugin.ExportResult, error)`, reporting the environment variables an external command needs. This is what backs `kevin connect`. `Export` must not create or change anything, only report how to reach what `Up` already created.
