...
`file`, `oci`, and `http` also accept `signing`, which pins provenance instead of (or alongside) a `checksum`/digest pin, under one of two schemes. `signing: scheme: "minisign"` requires a valid [minisign](https://jedisct1.github.io/minisign/) signature from a key in a local trust store (`~/.kevin/trusted-keys/`) before the package is ever extracted. Verification (Ed25519 plus BLAKE2b-512 prehashing, through minisign's own author's reference Go implementation) is the only signing code kevin ships for this scheme - signing itself is always the external `minisign` CLI, applied to the archive `kevin plugin pack` built, so kevin's binary never holds or generates secret key material. `signing: scheme: "sigstore"` requires a valid keyless [sigstore](https://www.sigstore.dev/) signature - a Fulcio-issued certificate matching a declared OIDC identity and issuer, with its Rekor inclusion proof checked offline - from an identity/issuer pair in a second local trust store (`~/.kevin/trusted-identities/`); kevin shells out to the external `cosign` CLI for both signing and verification (see [ADR-0007](https://github.com/justenwalker/kevin/blob/main/docs/adr/0007-shell-out-to-cosign-for-sigstore-verification.md)), so the same principle holds: kevin's binary never holds or generates the OIDC token or Fulcio-issued certificate either. Both trust stores live outside `kevin.cue` deliberately: `checksum` and a plugin's declared `oci`/`http` reference already live in the same file a project author controls, so pinning trust there too would let anyone who can edit `kevin.cue` also choose what's trusted - a `signing.identity`/`signing.issuer` pair in `kevin.cue` names *who should have signed*, not *who is trusted to have signed*. `kevin plugin trust add/list/remove` manage the minisign store, `add-identity`/`remove-identity` the sigstore one; `kevin plugin push` uploads a package's sibling `.minisig` or `.sigstore.json` file automatically, via a cosign-style fallback tag (`<repo>:sha256-<digest>.sig`). The OCI client kevin uses has no OCI 1.1 referrers-API support, so this reuses the same tag/manifest/blob calls the package itself already goes through, rather than a dedicated discovery mechanism.
