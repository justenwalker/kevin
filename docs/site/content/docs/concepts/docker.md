...
The engine creates the shared network before the DAG runs and removes it after. A container joins that network with a network alias equal to the step name, thus one step reaches another by step name. The network is dual-stack (`docker network create --ipv6`): a container gets both an IPv4 and, where the daemon assigns one, an IPv6 address, and the relay answers DNS and captures egress on whichever families it finds addresses in - see [Transparent capture]({{< relref "/docs/concepts/relay#transparent-capture" >}}).
