...
A route whose upstream already speaks TLS can opt out of this with `skip_mitm: true` (see [`builtin:route`]({{< relref "/docs/reference/steps/route" >}})): the CONNECT still resolves the route and still gets one entry in the traffic log, but instead of hijacking and hand-shaking, the proxy dials the upstream directly and splices bytes both ways, undecrypted. The client then validates the upstream's own certificate - a cert-manager-issued one inside a `builtin:kind` cluster, say - instead of a kevin-signed leaf. This only applies to a route whose upstream itself speaks TLS; a plain-HTTP upstream has no certificate to preserve and always needs the MITM described above to serve HTTPS to the client at all.
